...
export const deleteById: Middleware<State, PathParams, Ticket> = async (ctx) => {
  const ticket = await db('tickets').where('id', ctx.params.id).first();
  ...
  if (!value) {
    await db('ticket_events').insert({ event: 'INVALIDATE', ticket_id: ctx.params.id });
  }
